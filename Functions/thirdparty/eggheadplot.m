...
%       9    'MapStyle' - Color bar style [ 'haxby' | 'fireice' | 'bipolar' | 'hot' | 'cool' | 'autumn' | 'winter' | 'summer' | 'spring' | 'gray' | 'hotiso' | 'coldiso' | 'jet'  (default)]. 
...
%       14  'Steps' - Controls how many steps are included in the colormap.  Default is 512.
%       15  'Smooth' - Controls how many points are included for spatial smoothing. Default is 0.
%       16  'ShowElectrodes' - [ 'No' | 'Yes' (default) ].
%       17  'ShowBrain' - [ 'Yes' | 'No' (default)].
%       18  'BrainOpacity' - Controls how visible the brain appears with 0 being transparent and 1 being solid. Default is 0.23.
%      
%      
%   Example Code:
%       
...
%   Author: Matthew B. Pontifex, Health Behaviors and Cognition Laboratory, Michigan State University, August 27, 2014
    ...
    try, r.Channels; inchans = {r.Channels}; catch, bolerror = 1; end
    ...
    try, r.FontSize; handles.fontsize = r(1).FontSize; catch, handles.fontsize = 8; end
    try, r.MapStyle; handles.colormapstyle = r(1).MapStyle; catch, handles.colormapstyle = 'jet'; end
    ...
    try, r.Steps; handles.steps = r(1).Steps; catch, handles.steps = 512; end
    ...
    try, r.BrainOpacity; handles.BrainOpacity = r(1).BrainOpacity; catch, handles.BrainOpacity = 0.23; end
    ...
    %     handles.d = handles.d(1);
        ...
        for chann = temparray
            index = find(strcmp(handles.chanvect, chann));
            if ~(isempty(index))
        ...
        if (strcmpi(handles.ShowElectrodes, 'Yes'))
            handles.electrodefill = plot3(handles.origchanxvect*0.95,handles.origchanyvect*0.95,ones(size(handles.origchanxvect))*10,'.','Color',handles.markcolor,'markersize',handles.marksize,'linewidth',1); % Show electrodes
        end
        if (strcmpi(handles.ShowBrain, 'Yes'))
