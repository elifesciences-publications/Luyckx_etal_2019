...
%   revised for multipeaks - 3-25-2015 - mp
    ...
    if (r.opt == 1)
        ...
        if ((r.Latencies == 0) & (r.Window == 0))
            help erppeakintervalamplitude
            error('Error at erppeakintervalamplitude(). Either peak latency or window information must be provided.');
        end
        ...
        if (r.Latencies == 0)
            ...
            if (isempty(find(INERP.times == r.Window(1))) || isempty(find(INERP.times == r.Window(2)))) % Window period falls outside of the available epoch window
                msg = sprintf('Error at erppeakintervalamplitude(). Mismatch between the window period and the epoch size. A window period of %d to %d ms was specified but the epoch only contains data from %d to %d ms.', r.Window(1), r.Window(2), INERP.times(1), INERP.times(end));
                error(msg);
            end
            ...
            if (boolerr == 1)
                help geterpvalues
                msg = sprintf('Error at erppeakintervalamplitude() for geterpvalues(). There are not enough points for the specified neighborhood window outside of the specified window period.');
                error(msg);
            end        
        ...
        if (numel(r.Latencies) ~= numel(r.Channels))
            help erppeakintervalamplitude
            error('Error at erppeakintervalamplitude(). Peak latency information should be provided for each channel, the number of peaks and latencies do not match. This error may also indicate that ERPLABs geterpvalues() had an error.');
        end
        ...
        for rC = 1:numel(r.Points)
            if ((r.Points(rC) + r.Interval(1)) < 0) || ((r.Points(rC) + r.Interval(2)) > INERP.pnts)
                help erppeakintervalamplitude
                msg = sprintf('Error at erppeakintervalamplitude(). The interval period for channel %d falls outside the available points, use a different latency or narrow the interval window.', rC);
                error(msg);
            end
        end
