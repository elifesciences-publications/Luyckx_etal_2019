function [OUTEEG, com] = pop_visualinspectarray(INEEG)
    ...
    g1 = [0.5 0.5 ];
    s1 = [1];
    geometry = { g1 s1 g1 s1 g1 s1 g1 s1 g1 g1 g1 s1 s1 s1 s1 s1 s1 s1 s1 };
    uilist = { ...
      ...
      if ~isempty(structout)
          ...
          com = sprintf('\nRunning:\n\t%s = visualinspectarray(%s, ''ChannelScale'', ''%s'', ''Polarity'', ''%s'', ''Average'', ''%s'', ''Smooth'', ''%s'', ''guiSize'', %s, ''guiFontSize'', %d);\n', inputname(1), inputname(1), structout.ChannelScale, structout.Polarity, structout.Average, structout.Smooth, mat2str(structout.guiSize), structout.guiFontSize);
          disp(com)
          OUTEEG = visualinspectarray(INEEG, 'ChannelScale', structout.ChannelScale, 'Polarity', structout.Polarity, 'Average', structout.Average, 'Smooth', structout.Smooth,'guiSize', structout.guiSize, 'guiFontSize', structout.guiFontSize);
