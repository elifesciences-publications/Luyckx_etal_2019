...
for c = 1:2 % Per task
    ...
    s2 = 0; % increment to keep track of sub index
    
    for f = 1:nframes % Per frame (or collapsed)
        
        % Number subjects per frame (n = 46 when collapsed)
        nsub = sum(whichFrame == frameid(f));
        
        for s = 1:nsub % Per subject
            
            s2      = s2 +1; % keep track of subjects
            
            submat  = setdiff(1:nsub,s); % leave one out
            tmpcpp  = cppdat(whichFrame == frameid(f),:,:,c);
            ...
            timewdw(s2,1,c)	= timez(1);
            timewdw(s2,2,c) = timez(end);
        end
    end
end
...
for c = 1:2 % iterate over tasks
    for f = 1:nframes % iterate over frames
        whichTime   = round(median(timewdw(whichFrame == frameid(f),1,c))):round(median(timewdw(whichFrame == frameid(f),2,c))); % get activity from median time window
        plotdat     = squeeze(mean(mean(normdat(whichFrame == frameid(f),:,whichTime,whichSamp,c),1),3));
...
for c = 1:2
    for f = 1:nframes
        ...
        figR = figure;
        [steh] = fEEG_steplot(cppdat(whichFrame == frameid(f),:,:,c),timepoints,ylims,signif,masstest,erpcol,'bottom');
